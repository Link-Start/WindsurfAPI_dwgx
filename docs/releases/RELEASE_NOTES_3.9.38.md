...
本说明描述的是**实际发布的** `v3.9.38`：它包含 omp 的 corrections、其后的独立复核修正，
以及随复核重锚的锚点和两处测试夹具的时序修复。发布前 `v3.9.38` 曾两次指向别的提交
（`4e429fa`，随后 `27170ff`），两次都在**推送后、发行物产出前**被独立复核或 CI 否掉，
因此**没有任何版本曾以此前的指向发布**；远端最终只有本说明对应的提交。
本版 tag 由维护者移动过，这件事写在明处而不是留给读者去比对。
