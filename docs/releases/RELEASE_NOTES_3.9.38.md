...
本说明描述实际发布的 `v3.9.38`，最终 tag 指向 `0b78217`。发布链分开记录：

- `4e429fa` 是未推送的本地候选，已被独立审计要求修正。
- `27170ff` 推送后触发 Release #186（35773633801），测试失败，发行物作业跳过。
- `1ed5d18` 又触发 Release #187（35776846089），该轮取消，不能漏记这个中间指向。
- `0b78217` 的 Release #188（35778314521）六项作业成功；GitHub Release 于
  2026-09-22 20:16:40 UTC 发布，四项附件已上传。

维护者在发布前移动过 tag；本次独立复核没有移动 tag，也没有执行 push 或发布。
以上是 GitHub Release/发行物的状态，不声称此前推送的源码或 tag 从未被任何人取得。
既已发布，后续代码修正应使用新的版本与 tag，不覆盖 `v3.9.38` 的身份。
